			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						// var regPatt = /\|.*/g;
						var yourNumbers = getYourNumbers(scenario.split('|')[1]);
						var yourPrizes = getYourNumbersInfo(yourNumbersWhole, 1);
						var yourNeighbourWins = getYourNeighbourInfo(yourNumbersWhole);
						var yourBonusSymbs = getYourNumbersInfo(yourNumbersWhole, 3);
						var yourTriggerSource = getYourTriggersInfo(yourNumbersWhole);
						...
						function getPhasesData(A_arrGridData, A_arrPrizeData)
						{
							...
							{
								yourWins[yourWinsIndex] = checkMatch(winNumbers, A_arrGridData[yourWinsIndex]);
							}
						...
						var mgPhases = getPhasesData(yourNumbers, yourPrizes); 
						...
						const arrCellBackgroundDesc   = ['', 'matchesWinningNumberDesc', 'instantWinDesc', 'prizeBurstWinDesc'];
						...
						r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');
						r.push('<tr class="tablebody">');
						for (var luckyIndex = 0; luckyIndex < winNumbers.length; luckyIndex++)
						{
							symbLetter   = winNumbers[luckyIndex];
							canvasIdStr  = 'cvsWinningSymb' + symbLetter;
							elementStr   = 'keyWinningSymb' + symbLetter;
						...
						showGridSymbs(canvasIdStr, elementStr, yourNumbers, yourPrizes, yourWins);
						...
						for (var gameIndex = 0; gameIndex < orderedWins.length; gameIndex ++)
						{
							...
							symbLetter    = yourNumbers[orderedWins[gameIndex] - 1];
					...
					function getYourNumbers(lettersData)
					{
						var result = [];
						var turnData = lettersData.split(':');
						for (i = 0; i < turnData.length; i++)
						{
							result.push(turnData[i].split(',')[0]);
						}
						return result;
					}

					function getYourNumbersInfo(lettersData, index)
					{
						var result = '';
					...
					// Output: 1 number is in the drawn numbers, 2 if IW, 0 if no win
					function checkMatch(drawnNumbers, number)
					{						
